...
async function assertAllowedPublicUrl(source: TraineeImportSourceDefinition, value: string) {
  const url = new URL(value)
  if (url.protocol !== "https:" || url.username || url.password) {
    throw new Error("Source URL must be a public HTTPS address")
  }
  if (url.port && url.port !== "443") throw new Error("Source URL uses a blocked port")

  const hostname = url.hostname.toLowerCase()
  if (!source.allowedHosts.some((host) => hostname === host || hostname.endsWith(`.${host}`))) {
...
export async function fetchApprovedTraineeSourceText(
  ...
  for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount += 1) {
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      if (!location || redirectCount === MAX_REDIRECTS) throw new Error("Source redirected too many times")
