...
export function OpportunityDetail({ record, kind, related }: { record: SeoThesis | SeoProgram; kind: "master" | "phd" | "trainee"; related: Array<{id:string;title:string;organization:string}> }) {
  ...
  const active = isDeadlineOpen(record.deadline)
  ...
  return <PublicLayout>
    ...
    <header className="bg-primary px-4 py-12 text-primary-foreground"><div className="mx-auto max-w-5xl">
      ...
      <p className="mt-4 text-sm">Posted <time dateTime={datePosted}>{datePosted}</time> · Deadline: {record.deadline ? <time dateTime={record.deadline}>{deadlineLabel(record.deadline)}</time> : deadlineLabel(null, record.deadline_type)}</p>
    </div></header>
    <div className="mx-auto grid max-w-5xl gap-8 px-4 py-10 lg:grid-cols-[1fr_280px]">
      <article className="min-w-0 space-y-8">
        ...
        <section><h2 className="mb-3 text-2xl font-semibold">Funding and compensation</h2><p>{record.compensation === "paid" ? "Paid position" : record.compensation === "stipend" ? "Stipend offered" : record.compensation === "unpaid" ? "Unpaid position" : "Compensation not specified"}. See the original posting for amounts, duration and conditions.</p></section>
        ...
        <section><h2 className="mb-3 text-2xl font-semibold">How to apply</h2><p className="mb-4">{active ? `Submit your application through ${organization}'s official application process${record.deadline ? ` before ${deadlineLabel(record.deadline)}` : '. Apply early as a closing date has not been specified'}.` : "The advertised deadline has passed."}</p>
