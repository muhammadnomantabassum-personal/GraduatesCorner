...
export function OpportunityCard({ item, description, sourceLabel }: { item: ComparisonItem; description: string; sourceLabel?: string }) {
  ...
  const deadline = new Date(item.deadline ? item.deadline + (item.deadline.includes("T") ? "" : "T23:59:59Z") : Number.NaN)
  const days = Math.ceil((deadline.getTime() - now) / 86400000)
  const date = deadlineLabel(item.deadline, item.deadlineType)
  return <article className="opportunity-card group flex h-full min-w-0 flex-col rounded-2xl border border-border/80 bg-card p-5 sm:p-6">
    ...
    <div className="mt-5 flex flex-wrap gap-x-4 gap-y-2 text-sm text-muted-foreground"><span className="inline-flex items-center gap-1.5"><MapPin className="size-4 shrink-0" />{item.location}</span>{(item.compensation === "paid" || item.compensation === "stipend") && <span className="font-medium text-primary">{item.compensation === "stipend" ? "Stipend offered" : "Paid"}</span>}</div>
