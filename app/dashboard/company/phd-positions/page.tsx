...
export default function CompanyPhDPositionsPage() {
  const { user } = useAuth()
  ...
  const fetchMyTheses = useCallback(async () => {
    ...
    if (error) {
      toast.error("Failed to fetch your PhD positions")
    } else {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link href="/dashboard/company/phd-positions/new">
        ...
        {tabs
          ...
          .map((tab) => (
            <button
              ...
              className={`flex shrink-0 flex-1 items-center justify-center gap-1.5 whitespace-nowrap rounded-md px-3 py-1.5 text-xs font-medium transition-colors ${
                activeTab === tab.key
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="mt-4 text-muted-foreground text-sm">Loading your PhD positions...</p>
        </div>
      ) : filtered.length > 0 ? (
                    ...
                    <div className="mt-3 flex flex-col gap-1.5 text-xs text-muted-foreground">
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5">
                        ...
                        <span className="flex items-center gap-1.5">
                          ...
                          Deadline: {formatDeadline(thesis.deadline)}
                        </span>
                      </div>
                    </div>
                  </div>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="mb-1 text-base font-semibold text-foreground">
              {activeTab === "all" ? "No PhD positions posted yet" : `No ${activeTab} PhD positions`}
            </h3>
            <p className="mb-5 text-sm text-muted-foreground">
              {activeTab === "all"
                ? "Start by posting your first PhD position"
                : `You don't have any ${activeTab} PhD position postings`}
            </p>
            {activeTab === "all" && (
              <Link href="/dashboard/company/phd-positions/new">
                <Button className="gap-1.5">
                  <Plus className="h-4 w-4" />
                  Post Your First PhD Position
                </Button>
              </Link>
            )}
          </CardContent>
