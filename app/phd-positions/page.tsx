...
import { OpportunityTypeLinks } from "@/components/shared/opportunity-type-links"
...
export default function PhDPositionsPage() {
  ...
  useBrowseUrl({ search, setSearch, sort, setSort, filters, setFilters })
    ...
    <PublicLayout>
      <section className="relative overflow-hidden px-4 py-16 text-primary-foreground lg:py-20">
        <div
          ...
          style={{ backgroundImage: "url('https://images.unsplash.com/photo-1532094349884-543bc11b234d?q=80&w=2200&auto=format&fit=crop')" }}
        />
        <div className="absolute inset-0 bg-[linear-gradient(105deg,rgba(13,50,36,0.94)_0%,rgba(52,168,83,0.82)_46%,rgba(66,133,244,0.52)_100%)]" />
        <div className="relative mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1fr_24rem] lg:items-end">
          <div>
            <Badge className="mb-5 gap-2 border border-white/20 bg-white/12 text-white backdrop-blur">
              ...
              Research opportunity lab
            </Badge>
            <h1 className="max-w-4xl text-balance text-4xl font-bold tracking-tight lg:text-6xl">PhD Positions</h1>
            <p className="mt-5 max-w-2xl text-pretty text-lg leading-relaxed text-white/82">
              Explore doctoral positions and research roles with professional filters, topic intelligence, and fast shortlist tools.
            </p>
            <div className="mt-8 flex flex-wrap gap-2">
              ...
              )) : ["Machine learning", "Medicine", "Robotics", "Climate"].map((subject) => (
                <button key={subject} onClick={() => setSearch(subject)} className="rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-xs font-semibold text-white backdrop-blur hover:bg-white/18">
                  {subject}
                </button>
              ))}
          ...
          <div className="rounded-2xl border border-white/18 bg-white/12 p-5 shadow-[0_24px_80px_rgba(0,0,0,0.16)] backdrop-blur-xl">
            <p className="text-sm font-semibold text-white/78">Research intelligence</p>
            <div className="mt-4 grid grid-cols-2 gap-3">
              <HeroMetric icon={GraduationCap} label="Open roles" value={theses.length} />
              <HeroMetric icon={Building2} label="Institutions" value={organizationCount} />
              <HeroMetric icon={MapPin} label="Locations" value={locationCount} />
              <HeroMetric icon={Microscope} label="Funded" value={paidCount} />
            </div>
          </div>
        </div>
      </section>

      <section className="sticky top-16 z-30 -mt-8 px-4 pb-6 lg:top-16">
        <div className="mx-auto max-w-7xl rounded-2xl border border-border bg-card/95 p-3 shadow-[0_24px_80px_rgba(52,168,83,0.16)] backdrop-blur">
            ...
            <Sheet open={showFilters} onOpenChange={setShowFilters}>
              ...
              <SheetContent side="bottom" className="max-h-[86vh] overflow-y-auto rounded-t-2xl p-0">
                <SheetHeader className="border-b border-border p-4 text-left">
                  <SheetTitle>Refine PhD positions</SheetTitle>
                </SheetHeader>
                <div className="p-4">
                  <FilterPanel
                ...
                <div className="sticky bottom-0 border-t bg-card p-4 pb-[max(1rem,env(safe-area-inset-bottom))]"><Button className="min-h-12 w-full" onClick={() => setShowFilters(false)}>Show {filtered.length} opportunities</Button></div>
              </SheetContent>
            </Sheet>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mb-6 rounded-2xl border border-border bg-card p-5 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold text-foreground">Curated PhD results</h2>
                <p className="text-sm text-muted-foreground">
                  Showing <span className="font-semibold text-foreground">{filtered.length}</span> {filtered.length === 1 ? "position" : "positions"} from research-focused organizations
                </p>
              </div>
              <div className="flex items-center gap-2">
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-card py-20 text-center">
                  ...
                  <p className="mb-4 text-sm text-muted-foreground">
                    Try a broader topic or remove a location filter to see more opportunities.
                  </p>
